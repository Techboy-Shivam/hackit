...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $dob && $gender && $blood_group) {
        $stmt = $pdo->prepare("INSERT INTO patients
            (tenant_id, name, dob, gender, blood_group, contact, address, patient_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
            $tenant_id,
            $name,
...
<html>
<head>
  <title>HackIt NeoCare – Registration</title>
...
<div class="shell">
  <div class="panel">
    ...
    <div class="panel-sub">
      Quick patient registration for your hospital tenant in a single, smooth form.
    </div>
